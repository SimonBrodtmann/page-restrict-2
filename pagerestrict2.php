...
function pr_is_post_restricted() {
	if (pr_get_opt('method') == 'none') return false;
	global $post;
	$pr_check = pr_get_opt('method') == 'all';
	$pr_check = $pr_check || in_array($post->ID, pr_get_opt('pages')) || in_array($post->ID, pr_get_opt('posts'));
...
function pr_page_restrict ( $pr_page_content ) {
	...
	if (pr_is_post_restricted() && !is_user_logged_in()) {
